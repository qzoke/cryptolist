...
const getPercentageChange = (markets, btcNode, quoteSymbol) => {
  ...
  if (market && market.ticker)
    return (market.ticker.percentChange).toFixed(2);
  ...
  const btcQuoteMarket = btcNode.markets.data.find(market => market.marketSymbol.endsWith(quoteSymbol));
  if ((currencyBtcMarket && btcQuoteMarket) && currencyBtcMarket.ticker)
    return ((btcQuoteMarket.ticker.percentChange + currencyBtcMarket.ticker.percentChange)).toFixed(2);
...
const get24HourVolume = (market, btcMarket, priceOfBtc, quoteSymbol) => {
  if (market && market.ticker)
    return asCurrency(market.ticker.quoteVolume, quoteSymbol);
...
export const marketCapFormat = (currencies, btcNode, quoteSymbol) => {
  ...
  return currencies.map(currency => {
    ...
    const market = hasMarkets ? currency.markets.data.find(market => market.marketSymbol.endsWith(quoteSymbol)) : null;
